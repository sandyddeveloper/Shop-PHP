...
// Initialize goods_display
$goods_display = "";

// Check for order_id in URL parameters
if (isset($_GET['id']) && !empty($_GET['id'])) {
    $get_order_id = $_GET['id'];
    $is_direct_lookup = true; // Flag to check direct lookup
} else {
    $get_order_id = $_POST['get_order_id'] ?? '';
    $is_direct_lookup = false; // Normal form submission
}



if (!empty($get_order_id)) {
    $sql = $conn->prepare("SELECT `prod_ids` FROM `orders` WHERE `get_order_id` = ?");
    $sql->bind_param("s", $get_order_id);
    $sql->execute();
    $result = $sql->get_result();

    if ($row = $result->fetch_assoc()) {
        $prod_ids = json_decode($row['prod_ids'], true);
        $goods_display = "";
        
        foreach ($prod_ids as $prod) {
            $goods_display .= "Product Name: " . $prod['title'] . "\n";
            $goods_display .= "Accounts:\n" . implode("\n", $prod['goods']) . "\n\n";
        }
    } else {
        $goods_display = "Order ID not found.";
    }

    $conn->close();
} else if (!$is_direct_lookup) {
    $goods_display = "Please enter an Order ID.";
}
...
<head>
    ...
    <title>Polish - Order Lookup</title>
    <style>
   body {
        color: #131313;
    ...
    .container {
        ...
        width: 600px; /* Container width to ensure responsiveness */
        max-width: 800px;
        margin-top: 60px;
    }
    textarea {
    ...
    input[type="text"] {
        padding: 10px;
        width: calc(90% - 22px); /* Adjust width considering padding and border */
        border-radius: 5px;
        border: 1px solid #555;
        margin-top: 20px;
    }
...
<body style="
        color: #131313;
        background-color: #131313;
        background: #131313;  display: flex;
        justify-content: center;
        align-items: center;
        margin: 0;">
<div class="stars-background-def small-def"></div>
            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-9">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="dashboard_main_head text-center" style="margin-left: 25px !important">
                                    <h5>Deliver Order</h5>
                                    <p>You can pick up your order here</p>
                                </div>
                            </div>
                        </div>
                        <?php
    ...
    <?php if (!$is_direct_lookup): ?>
    <form method="post">
        <label for="get_order_id" style="color: #FFF !important">Enter Unique ID:</label><br>
        <input type="text" id="get_order_id" name="get_order_id" required><br>
        <input type="submit" value="Lookup Order">
    </form>
    <?php endif; ?>

    <?php if (!empty($goods_display)): ?>
        <textarea id="goodsText" readonly><?= htmlspecialchars($goods_display) ?></textarea><br>
